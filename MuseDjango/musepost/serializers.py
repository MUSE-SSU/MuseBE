...
class PostDisplayMyPageLikeSerializer(serializers.ModelSerializer):
    ...
    def get_posts(self, obj):
        posts = Post.objects.filter(idx=obj.post)
        try:
            login_user = self.context.get("request").user
        except:
            login_user = None
        post = PostDisplayMyPageSerializer(posts, context={"request": login_user}, many=True)
        return post.data
